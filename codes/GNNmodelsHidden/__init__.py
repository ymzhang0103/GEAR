...
class GnnNets(nn.Module):
    ...
    def forward(self, data):
        data = data.to(self.device)
        logits, prob, emb, graph_emb, h_all = self.model(data)
        return logits, prob, emb, graph_emb, h_all
    
    def to_device(self):
        self.to(self.device)

class GnnNets_NC(nn.Module):
    def __init__(self, input_dim, output_dim, model_args):
    ...
    def forward(self, data):
        data = data.to(self.device)
        logits, prob, emb, h_all = self.model(data)
        return logits, prob, emb, h_all
