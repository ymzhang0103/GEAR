...
def build_graph(
    ...
    for shape_id, shape in enumerate(list_shapes):
        ...
        for r,c in graph_s.edges():
            shape_edges_rows.append(r)
            shape_edges_cols.append(c)
        if shape_type == "cycle":
    ...
    nodes_num = nx.number_of_nodes(basis)
    edge_labels = sp.coo_matrix((np.ones(len(shape_edges_rows)), (np.array(shape_edges_rows), np.array(shape_edges_cols))), shape=(nodes_num, nodes_num))
    return basis, role_id, plugins, edge_labels
...
def build_graph_new(
    ...
    for shape_id, shape in enumerate(list_shapes):
        ...
        basis.add_edges_from([(start+a, plugins[shape_id]+b)])
        basis.add_edges_from([(start+b, plugins[shape_id]+a)])
        '''
        for i in range(add_edge_num):
            while True:
                print(seed)
                np.random.seed(seed)
                u = np.random.randint(0, 5)
                v = np.random.randint(0, len(plugins))
                print("u",u,"v",v)
                seed = seed+1
                if (not basis.has_edge(u, v)) and (u != v):
                    break
            basis.add_edges_from([(start+u, plugins[v])])
